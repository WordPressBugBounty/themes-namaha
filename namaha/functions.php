 ...
 * @package Namaha
 */
define( 'NAMAHA_THEME_VERSION' , '1.0.71' );
...
/**
 * Tweak custom logo image attributes (free + premium).
 */
add_filter( 'get_custom_logo_image_attributes', function( $attr, $blog_id ) {

	// Safety on multisite – don't touch other sites' logos.
	if ( is_multisite() && (int) $blog_id !== get_current_blog_id() ) {
		return $attr;
	}

	// Use site name as alt text (cleaner than "Site – Tagline").
 ...
 * }
 */
function namaha_get_black_friday_window() {
 ...
 */
function namaha_black_friday_notice() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}

	$window = namaha_get_black_friday_window();
	$today  = namaha_today_ymd();
	...
	$message = array(
		...
		'heading' => __( 'Black Friday Weekend Sale', 'namaha' ),
		'text'    => sprintf(
			__( '<a href="%1$s" target="_blank"><span style="font-size: 20px">🖤</span>Get 40%% off any of our Premium WordPress themes this Black Friday weekend!<span style="font-size: 20px">🖤</span></a>', 'namaha' ),
	...
	if ( ! empty( $message['text'] ) && get_user_meta( $user_id, 'namaha_admin_notice_' . $message['id'] . '_dismissed', true ) ) {
		return;
	}
	...
	$dismiss_url = wp_nonce_url(
		add_query_arg(
			array(
				'namaha-admin-notice-dismissed' => '1',
				'namaha-admin-notice-id'        => $message['id'],
			)
		),
		'namaha_dismiss_notice_' . $message['id']
	);
	...
	printf(
		...
		esc_html__( 'Read More', 'namaha' ),
		esc_url( $dismiss_url ),
		esc_html__( 'Dismiss', 'namaha' )
	);
}
add_action( 'admin_notices', 'namaha_black_friday_notice' );

/**
 * Show Cyber Monday–only admin notice.
 */
function namaha_cyber_monday_notice() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}
	...
	$message = array(
		...
		'heading' => __( 'Cyber Monday Sale', 'namaha' ),
		'text'    => sprintf(
			__( '<a href="%1$s" target="_blank"><i class="fas fa-terminal"></i>Cyber Monday specials activated… <span class="otb-cursor" aria-hidden="true"></span></a>', 'namaha' ),
...
add_action( 'admin_notices', 'namaha_cyber_monday_notice' );

function namaha_christmas_notice() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}
	...
	$message = array(
		'id'      => 'christmas_' . $year,
		'heading' => __( 'Christmas Sale', 'namaha' ),
		'text'    => sprintf(
			__( '<a href="%1$s" target="_blank"><span style="font-size: 20px">🎄</span>Get 20%% off any of our Premium WordPress themes until Christmas Day!<span style="font-size: 20px">🎄</span></a>', 'namaha' ),
...
add_action( 'admin_notices', 'namaha_christmas_notice' );
...
function namaha_add_body_class( $classes ) {
	...
	if ( !get_theme_mod( 'namaha-show-recaptcha-badge', customizer_library_get_default( 'namaha-show-recaptcha-badge' ) ) ) {
	...
	if ( get_theme_mod( 'namaha-bbpress-use-theme-styles', customizer_library_get_default( 'namaha-bbpress-use-theme-styles' ) ) ) {
		$classes[] = 'namaha-bbpress-use-theme-styles';
	}
	
	if ( get_theme_mod( 'namaha-bookingpress-use-theme-styles', customizer_library_get_default( 'namaha-bookingpress-use-theme-styles' ) ) ) {
		$classes[] = 'namaha-bookingpress-use-theme-styles';
	}
...
function namaha_singular_or_plural( $singular, $plural, $value ) {
	$locale = get_locale();
